...
$action = base_url('blogs/store');

if($data_edit){
  $action = base_url('blogs/update');
}
...
<form method="POST" action="<?= $action ?>" id="upload-create" enctype="multipart/form-data" class="form-horizontal">
  <div class="box-body">
    ...
    <div class="form-group">
      <label for="inputEmail3" class="col-sm-3 control-label">Judul Blog / Informasi*</label>
      <div class="col-sm-9">
        <?php if($data_edit){echo '<input type="hidden" name="dt[id]" value="'.$data_edit['id'].'">'; }  ?>
        <input type="text" class="form-control" placeholder="Masukan Judul Blog / Informasi ..." name="dt[title]" <?php if($data_edit){echo "value='".$data_edit['title']."'"; }  ?>>
      </div>
    </div>
    <div class="form-group">
      <label for="inputEmail3" class="col-sm-3 control-label">Gambar Blog / Informasi</label>
      <div class="col-sm-9">
        <div class="row">
  ...
  <div class="box-footer" align="right">
    <a href="<?= base_url('blogs') ?> ">
      <button type="button" class="btn btn-info"><i class="fa fa-stars"></i> Data Blog / Informasi</button>
    </a>
    <?php if($data_edit){ ?>
